plot_for_diff_bars <- function(collapsed_df, comps, grouping_factors, col_name, plot_type="scatter", emmeans=NULL,
                                colors=NULL, first_grouping_factor_displayname=NULL, fill_factor=NULL,
                                outline_factor=NULL, patterns=NULL,
                                ...
                                fig_dims=NULL, show_p_values=TRUE, horz_offset=0.008) {
  ...
  # Get the x & y values that are actually being plotted.
    ...
    # Plot the data
    if (plot_type == "bar") {
      gp <- my_bar_plot(collapsed_df, grouping_factors, first_grouping_factor_displayname, 
      ...
      gp <- my_scatter_plot(collapsed_df, grouping_factors, first_grouping_factor_displayname, 
                            col_name, colors=colors, patterns=patterns, fill_factor=fill_factor, 
                            outline_factor=outline_factor, x_axis_factors=x_axis_factors)
    }
...
plot_for_diff_bars(collapsed_df, comps, plot_grouping_factors, col_name, plot_type = "scatter", emmeans=emmeans)