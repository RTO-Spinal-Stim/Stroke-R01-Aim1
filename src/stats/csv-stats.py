...
for speed in speeds:
    for table_name in table_names:
        ...
        save_path_sham = Path("results/stats/ttest_results/sham_vs_zero/symmetry/{{data_column}}_{speed}_ttest.pdf".format(speed=speed))
        ...
        save_path_stim = Path("results/stats/ttest_results/stim_vs_zero/symmetry/{{data_column}}_{speed}_ttest.pdf".format(speed=speed))
        ...
        for column in all_df_columns:
            ...
            best_stim_vs_sham_save_path = Path("results/stats/ttest_results/best_stim_vs_sham/symmetry/{{data_column}}_{speed}_ttest.pdf".format(speed=speed))
            best_stim_vs_sham_save_path.parent.mkdir(parents=True, exist_ok=True)
            results = ttest_dep(best_stim_and_sham_df, group_column_name, "_", repeated_measures_column=repeated_measures_column, filename=best_stim_vs_sham_save_path)
            worst_stim_and_sham_df = pd.concat([sham_df, worst_stim_df])
            worst_stim_vs_sham_save_path = Path("results/stats/ttest_results/worst_stim_vs_sham/symmetry/{{data_column}}_{speed}_ttest.pdf".format(speed=speed))
            worst_stim_vs_sham_save_path.parent.mkdir(parents=True, exist_ok=True)
            results = ttest_dep(worst_stim_and_sham_df, group_column_name, "_", repeated_measures_column=repeated_measures_column, filename=worst_stim_vs_sham_save_path)            
            # Perform one sample best and worst STIM t-test
            save_path_best_stim = Path("results/stats/ttest_results/best_stim_vs_zero/symmetry/{{data_column}}_{speed}_ttest.pdf".format(speed=speed))
            save_path_best_stim.parent.mkdir(parents=True, exist_ok=True)
            results = ttest_ind(best_stim_df, group_column_name, column, filename=save_path_best_stim)
            save_path_worst_stim = Path("results/stats/ttest_results/worst_stim_vs_zero/symmetry/{{data_column}}_{speed}_ttest.pdf".format(speed=speed))
            save_path_worst_stim.parent.mkdir(parents=True, exist_ok=True)
            results = ttest_ind(worst_stim_df, group_column_name, column, filename=save_path_worst_stim)
