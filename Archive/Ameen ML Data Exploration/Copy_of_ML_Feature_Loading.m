...
% Define filenames for each table
csvFileNameSSV = 'MLresults_SSV.csv';
csvFileNameFV = 'MLresults_FV.csv';

% Check if the SSV file exists before loading
if isfile(csvFileNameSSV)
    % Load the SSV table if the file exists
    resultsTableSSV = readtable(csvFileNameSSV);
    disp('Loaded existing SSV results table.');
else
    % Initialize an empty table for SSV if the file does not exist
    disp('No existing SSV results table found. Initializing a new table.');
    resultsTableSSV = table();
end

% Check if the FV file exists before loading
if isfile(csvFileNameFV)
    % Load the FV table if the file exists
    resultsTableFV = readtable(csvFileNameFV);
    disp('Loaded existing FV results table.');
else
    % Initialize an empty table for FV if the file does not exist
    disp('No existing FV results table found. Initializing a new table.');
    resultsTableFV = table();
end
...
columnNames = {'SubjectID', 'StimNoStim','Intensity', 'Frequency', 'Velocity', 'Trial','GaitCycle', 'delta_StepLenSym', 'delta_SwingTimeSym', 'delta_StrideVelocitySym','delta_SynergiesNeeded'};
%Add Muscle and Joint feature headers
for m = muscles
    columnNames = [columnNames, strcat(m, '_delta_rmsSymmetry'), strcat(m, '_delta_aucSymmetry'), ...
                   strcat(m, '_delta_maxCorr'), strcat(m, '_delta_peakLag')];
end
for j = joints
    columnNames = [columnNames, strcat(j, '_delta_rmsSymmetry'), strcat(j, '_delta_aucSymmetry'), ...
                   strcat(j, '_delta_maxCorr'), strcat(j, '_delta_peakLag'), strcat(j, '_delta_romSymmetry')];
end

if ~ isfile(csvFileNameSSV)
% Initialize the results table
resultsTableSSV = table('Size', [0, numel(columnNames)], 'VariableTypes', repmat({'double'}, 1, numel(columnNames)), 'VariableNames', columnNames);
end

if ~ isfile(csvFileNameFV)
% Initialize the results table
resultsTableFV = table('Size', [0, numel(columnNames)], 'VariableTypes', repmat({'double'}, 1, numel(columnNames)), 'VariableNames', columnNames);
end
    ...
    for j = 1:length(f)
        ...
        for t = 1:length(trials)
            ...
            for c = 1:N
                ...
                results.(velocity).(pre_post) = [results.(velocity).(pre_post);newRow];
